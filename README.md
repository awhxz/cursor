...
- `Актуальные`: GID `1654701914`
- `Общий бэклог`: GID `1200022421`
- `Готовые задачи`: GID `1363218094`

Дашборд обращается к `/api/sheets`, серверный обработчик пробует прочитать публичный CSV и возвращает JSON для интерфейса. Каждая вкладка запрашивает соответствующий лист по его GID, а вкладка «Все» объединяет данные трех листов. На вкладке «Актуальные» задачи сгруппированы по ответственным; «Бэклог» и «Готовые и стоп» выводятся общими списками. Данные не копируются в код и обновляются без нового деплоя. В верхней части интерфейса показывается ссылка на Google Sheets, откуда загружены данные.

Если используется service account, `GOOGLE_SHEET_GID` можно не указывать: GID рабочих листов уже заданы в конфигурации дашборда, а сервер получает их актуальные названия через Google Sheets API.
